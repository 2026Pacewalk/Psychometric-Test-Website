...
interface C {
  name: string; code: string; email: string; ownerName: string; mobile: string;
  city: string; district: string; state: string; address: string; status: string; commissionPercent: number;
}

export default function CentreProfileClient({ centre }: { centre: C }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    const res = await fetch("/api/centre/profile", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, ...pwd }) });
    setSaving(false);
  ...
  const set = (k: keyof C) => (e: React.ChangeEvent<HTMLInputElement>) => setForm((f) => ({ ...f, [k]: e.target.value }));

  return (
    <div className="max-w-3xl space-y-6">
      <div><h1 className="text-2xl font-extrabold text-slate-900">Centre Profile</h1><p className="text-sm text-slate-500">Manage your details and password. Commission: {centre.commissionPercent}% AMG / {100 - centre.commissionPercent}% you.</p></div>
      <form onSubmit={save} className="card space-y-5 p-6">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div><label className="label">Centre Name</label><input className="input" value={form.name} onChange={set("name")} /></div>
          <div><label className="label">Centre Code</label><input className="input bg-slate-50" value={form.code} readOnly /></div>
          ...
          <div><label className="label">Mobile</label><input className="input" value={form.mobile} onChange={set("mobile")} /></div>
          <div><label className="label">Status</label><input className="input bg-slate-50 capitalize" value={form.status} readOnly /></div>
          <div><label className="label">City</label><input className="input" value={form.city} onChange={set("city")} /></div>
          <div><label className="label">District</label><input className="input" value={form.district} onChange={set("district")} /></div>
